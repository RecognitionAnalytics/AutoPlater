...
def PlatingLoop(stage,plater, setParameters,platingChannels,repeatChannels,conductanceDatabase,dataSend):
    ...
    while len(platingChannels)>0:
        ...
        conductance_nA=RunIV(infos, channel,stage+ '_Before', 100, plater, conductanceDatabase,dataSend)
        plater.setBias(0)
...
def RunIV(infos, channel, stage,  maxVoltage_mV, plater, graphDialog, conductanceDatabase, dataSend, slew_mV_s=300, topElectrode=True):
    if topElectrode:
        plater.TopElectrode()
        electrode='Top'
    else:
    ...
    plater.ResetDevice()
    plater.disableThreshold()
    conductance_nA, outBias,currents =plater.runIV2( maxVoltage_mV=maxVoltage_mV, slew_mV_s=slew_mV_s, plot=False)
    
    graphDialog.update_IV( channel,outBias,currents) 
